...
const GlobalStyles = () => (
  <style>{`
    ...
    body { margin: 0; padding: 0; font-family: 'Inter', sans-serif; background-color: var(--bg-light); transition: background-color 0.3s ease; }
    .dark body { background-color: var(--bg-dark); color: #f8fafc; }
    .bento-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 1.5rem; padding: 2rem; }
    .scout-card {
      background: white; border-radius: 1.5rem; padding: 1.25rem; display: flex; flex-direction: column;
      align-items: center; justify-content: center; text-align: center; transition: all 0.3s ease;
      border: 1px solid rgba(0,0,0,0.05); position: relative; aspect-ratio: 1/1; text-decoration: none;
      color: inherit; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);
    }
    ...
    .image-container { width: 100%; height: 70%; position: relative; overflow: hidden; border-radius: 1rem; margin-bottom: 0.75rem; background: #f8fafc; display: flex; align-items: center; justify-content: center; }
    .dark .image-container { background: #0f172a; }
    .card-title { font-weight: 700; font-size: 0.9rem; text-transform: uppercase; letter-spacing: 0.025em; color: var(--bsa-blue); }
    .dark .card-title { color: #94a3b8; }
    .admin-badge { position: absolute; top: -0.5rem; right: -0.5rem; display: flex; gap: 0.25rem; z-index: 20; }
    .icon-btn { width: 2rem; height: 2rem; border-radius: 50%; display: flex; align-items: center; justify-content: center; border: none; cursor: pointer; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
  `}</style>
);
...
export default function App() {
  ...
  const saveData = async (newLinks: ScoutLink[], newSettings: AppSettings) => {
    ...
    try {
      const appId = window.__app_id || 'scout-links-v1';
      await window.firebase.firestore().collection('artifacts').doc(appId).collection('public').doc('config').set({
        links: newLinks,
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white transition-colors">
        ...
        <header className="bg-white dark:bg-slate-900 border-b-4 border-yellow-400 shadow-xl px-6 py-8 relative">
          <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-8">
            ...
            <div className="text-center md:text-left flex-1">
              ...
              <div className="text-lg font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center justify-center md:justify-start gap-2">
            ...
            <div className="flex gap-3">
              <button onClick={() => setDarkMode(!darkMode)} className="p-4 rounded-2xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-yellow-400 hover:text-blue-900 transition-all">
                {darkMode ? <Sun /> : <Moon />}
              </button>
              {isAdmin && (
                <button 
          ...
          <div className="bento-grid">
              ...
              <div key={link.id} className="relative group">
                {isEditing && (
                  <div className="admin-badge">
                    <button onClick={() => { setModalMode('link'); setFormData({ ...link }); setEditingLink(link); setIsModalOpen(true); }} className="icon-btn bg-blue-600 text-white"><Edit2 size={12} /></button>
                    <button onClick={() => saveData(links.filter(l => l.id !== link.id), settings)} className="icon-btn bg-red-600 text-white"><Trash2 size={12} /></button>
                  </div>
                )}
            ...
            {isEditing && (
              ...
              >
                ...
                <div className="font-black text-slate-400 uppercase text-[10px] tracking-widest">Add Link</div>
              </button>
            )}
          </div>
        </main>
        ...
        <div className="fixed bottom-8 right-8 z-[100]">
          <button 
            ...
            className={`w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition-all active:scale-90 ${isAdmin ? 'bg-yellow-400' : 'bg-blue-900'}`}
          >
          ...
          <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[300] flex items-center justify-center p-6">
            <div className="bg-white dark:bg-slate-800 p-10 rounded-[2.5rem] w-full max-w-sm shadow-2xl border-b-8 border-blue-900">
               ...
               <form onSubmit={handleLogin} className="space-y-4">
                  <input type="password" autoFocus placeholder="Password" value={passwordInput} onChange={e => setPasswordInput(e.target.value)} className="w-full p-5 rounded-2xl bg-slate-100 dark:bg-slate-900 border-none font-bold text-center" />
                  <div className="flex gap-3">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[3rem] w-full max-w-xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
              ...
              <form onSubmit={handleSaveModal} className="p-8 overflow-y-auto space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    {modalMode === 'link' ? (
                      <div>
                        ...
                        <input required value={formData.url || ''} onChange={e => setFormData({...formData, url: e.target.value})} className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl" placeholder="example.com" />
                      </div>
                    ) : (
                      <div>
                        <label className="text-[10px] font-black uppercase text-slate-400 mb-1 block">Subtitle</label>
                        <input value={formData.subtitle || ''} onChange={e => setFormData({...formData, subtitle: e.target.value})} className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl" />
                      </div>
                    )}
                    <div>
                      <label className="text-[10px] font-black uppercase text-slate-400 mb-1 block">Image URL</label>
                      <input value={formData.imageUrl || ''} onChange={e => setFormData({...formData, imageUrl: e.target.value, iconName: ''})} className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-xs" />
                    </div>
                  </div>
                  ...
                  <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
                    <div className="flex flex-wrap justify-center gap-2">
                      {Object.keys(ICON_MAP).map(n => {
                        ...
                        return (
                          <button key={n} type="button" onClick={() => setFormData({...formData, iconName: n, imageUrl: ''})} className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${formData.iconName === n ? 'bg-blue-900 text-white shadow-lg' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}>
                            <Icon size={18} />
                          </button>
                        );
                      })}
                ...
                <div className="flex gap-4 pt-4">
                   ...
                   <button type="submit" className="flex-[2] p-5 bg-blue-900 text-white rounded-[1.5rem] font-black uppercase tracking-widest shadow-xl">Apply</button>
                </div>
              </form>
            </div>
          </div>
